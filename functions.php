...
function p2_get_at_name_map() {
	...
	foreach( $name_map as $name => $values)
 		$name_map[$name]['replacement'] = '<a href="' . esc_url( '/mentions/'. get_usermeta($values['id'], 'user_login') ) . '">' . esc_html( $name ) . '</a>';
...
function p2_the_title($before = '<h2>', $after = '</h2>', $returner = false) {
	...
	if( ( false === $pos or $pos > 0) && $title != '' ) {
		$outputs = ( is_single() ) ? $before.$t.$after : $before.'<a href="'.get_permalink($temp->ID).'">'.$t.'&nbsp;</a>'.$after;
		if($returner == true) {
			return $outputs;
		} else {
			echo $outputs;
